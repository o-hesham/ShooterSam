...
void AShooterSamPlayerController::BeginPlay()
{
	...
	DamageFlashWidget = CreateWidget<UDamageFlashWidget>(this, UDamageFlashWidget::StaticClass());
	if (DamageFlashWidget)
	{
		DamageFlashWidget->SetVisibility(ESlateVisibility::HitTestInvisible);
		DamageFlashWidget->AddToViewport(10);
	}
}

void AShooterSamPlayerController::ShowDamageFlash()
{
	if (DamageFlashWidget)
	{
		DamageFlashWidget->PlayDamageFlash();
	}
}
